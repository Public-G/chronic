...
<form class="layui-form">
    <input type="hidden" name="locid" value="${param.locid}"/>
    <div class="layui-form-item" style="margin-top: 15px;">
        <label class="layui-form-label">机构编码</label>
        <div class="layui-input-inline">
            <input type="text" name="org_code" id="org_code" value="${param.org_code}" lay-verify="required|orgCodeVerify"
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text" class="layui-input" autocomplete="off" name="org_name" value="${param.org_name}" lay-verify="required">
        </div>
    </div>
    ...
    layui.use(['form', 'layer'], function () {
        ...
        $("#org_code").blur(function () {
            ...
            $.getJSON("http://www.chronic.com/permission/location_fieldVerify?org_code=" + org_code, function(data) {
                ...
                if (data.code === code_success) {
                    $('#tip').html("<i class='fa fa-frown-o' style='font-size: 16px; color: #FF5722;'><font color='#FF5722'> 机构编码已存在</font></i>");
                } else {
                    $('#tip').html("<i class='fa fa-smile-o' style='font-size: 16px; color: #FFB800;'><font color='#FFB800'> 机构编码可以使用</font></i>");
                }
            });
        });
        ...
        form.on('submit(location_submit)',function(data) {
            ...
            if (!submitBtn.hasClass("layui-btn-disabled")) {
                ...
                $.post("http://www.chronic.com/permission/location_update?forwardType=update",data.field,function(data) {
                    if (data.code === code_success) {
                        layer.msg(data.msg);
                        setTimeout(function() {
